...
{
    Task<Result<List<SprintDto>>> GetByProjectAsync(Guid companyId, int projectId, string userId, bool isCompanyAdmin, bool includeArchived, CancellationToken ct);
    Task<Result<SprintDto>> GetByIdAsync(Guid companyId, int sprintId, string userId, bool isCompanyAdmin, CancellationToken ct);
    ...
    Task<Result<SprintBoardDto>> GetBoardAsync(Guid companyId, int sprintId, string userId, bool isCompanyAdmin, CancellationToken ct);
    Task<Result<bool>> MoveTaskToActiveAsync(Guid companyId, int sprintId, int taskId, CancellationToken ct);
    Task<Result<bool>> SetArchivedAsync(Guid companyId, int sprintId, bool isArchived, CancellationToken ct);
}
